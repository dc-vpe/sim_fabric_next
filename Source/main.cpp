...
void DisplayTokenAsText(int64_t index, TokenTypes type)
{
    ...
    printf("%4d     0x%08x     %s", index, type, name);
    printf("\n");
}

/// \desc Displays the token at index if it exists.
/// \param index index of the token to display, displays (invalid index) if index is outside the valid range.
void DisplayTokenInfo(int64_t index)
{
    if ( index >= 0 && index < Lexer::TotalTokens() )
    {
        TokenTypes type = Lexer::GetTokenType(index);
        DisplayTokenAsText(index, type);
        return;
    }

    printf("%4d     0x%08x     (invalid index)\n", index, INVALID_TOKEN);
}

/// \desc function that simply displays the list of tokens generated by the lexer.
/// \param lexer
void DisplayTokens()
{
    printf("Token List:\n");
...
void Help()
{
    ...
    printf("-t2     Parser trace information.\n");
