...
///         like an error.
void PrintError(int64_t number, const char *msg, bool error, bool fatalError = false);

/// \desc This method is functionally the same as print issue. It is used when an error
///       number and message is passed to the print issue from arguments passed to its
///       paren method. Currently only used in skip to end of block.
/// \param number Error or warning number.
/// \param error True if this is an error, false if it is a warning.
/// \param fatalError True if this error force the lex or compile to stop.
/// \param format CString printf style format string message describing the error.
